"""
Backtest Current Improved Anomaly Strategy - May to October 2024 (6 months)
Runs separate backtests for each month
"""
import pandas as pd
...
def main():
    """Run monthly backtests for May to October 2024."""
    ...
    print('='*100)
    print('CURRENT STRATEGY - MONTHLY BACKTEST (MAY TO OCTOBER 2025)')
    print('='*100)
    ...
    print(f'\nRunning 6 separate monthly backtests (May - October 2025)...')
    
    end_date = datetime.now()
    all_results = []
    
    # Run backtest for each month (May through October)
    ...
    print(f'\n{"Month":<12} {"Period":<25} {"Return %":<12} {"Win Rate %":<12} {"Trades":<10} {"Profit":<15} {"Stop-L":<8} {"Trail":<8} {"Profitable":<12}')
    print('-'*130)
    
    for result in all_results:
        period_str = f"{result['start_date']} to {result['end_date']}"
        print(f"{result['month']:<12} {period_str:<25} "
    ...
    print(f'\n{"Month":<12} {"Cumulative Return %":<20} {"Cumulative Profit":<20} {"Cumulative Trades":<20}')
    print('-'*70)
    ...
    for i, result in enumerate(all_results, 1):
        ...
        print(f"{result['month']:<12} {cumulative_return:<19.2f}% ${cumulative_profit:<19.2f} {cumulative_trades:<20}")
    ...
    print(f'  Total 6-Month Return: {cumulative_return:.2f}%')
    ...
    print(f'  Total Trades (6 months): {sum(trades):,}')
    ...
    output_file = f"current_strategy_may_oct_2025_{timestamp}.csv"
