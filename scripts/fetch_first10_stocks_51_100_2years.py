"""
Fetch first 10 stocks from stocks 51-100 at 30-minute intervals for the past 2 years using Alpaca API.
"""
import pandas as pd
...
# First 10 stocks from stocks 51-100
FIRST_10_STOCKS = [
...
def main():
    ...
    print("=" * 80)
    print("FETCHING FIRST 10 STOCKS FROM STOCKS 51-100 (30-MINUTE INTERVALS) - 2 YEARS")
    print("Using: Alpaca API")
    print("=" * 80)
    print(f"Period: {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}")
    print(f"Interval: 30 minutes")
    print(f"Market Hours: 9:30 AM - 4:00 PM ET")
    print(f"Stocks: {len(FIRST_10_STOCKS)} stocks")
    print(f"Stocks: {', '.join(FIRST_10_STOCKS)}")
    print()
    ...
    try:
        ...
        for i, symbol in enumerate(FIRST_10_STOCKS, 1):
            print(f"[{i}/{len(FIRST_10_STOCKS)}] {symbol}:", end=' ')
        ...
        output_file = f"first10_stocks_51_100_30min_2years_alpaca_{end_date.strftime('%Y%m%d')}.csv"
