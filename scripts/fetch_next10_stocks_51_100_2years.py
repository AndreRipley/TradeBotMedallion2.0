"""
Fetch next 10 stocks (11-20) from stocks 51-100 at 30-minute intervals for the past 2 years using Alpaca API.
"""
import pandas as pd
...
# Next 10 stocks from stocks 51-100 (stocks 11-20, which are stocks 61-70 overall)
NEXT_10_STOCKS = [
...
def main():
    ...
    print("=" * 80)
    print("FETCHING NEXT 10 STOCKS (11-20) FROM STOCKS 51-100 (30-MINUTE INTERVALS) - 2 YEARS")
    print("Using: Alpaca API")
    print("=" * 80)
    print(f"Period: {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}")
    print(f"Interval: 30 minutes")
    print(f"Market Hours: 9:30 AM - 4:00 PM ET")
    print(f"Stocks: {len(NEXT_10_STOCKS)} stocks")
    print(f"Stocks: {', '.join(NEXT_10_STOCKS)}")
    print()
    ...
    try:
        ...
        for i, symbol in enumerate(NEXT_10_STOCKS, 1):
            print(f"[{i}/{len(NEXT_10_STOCKS)}] {symbol}:", end=' ')
        ...
        output_file = f"next10_stocks_51_100_30min_2years_alpaca_{end_date.strftime('%Y%m%d')}.csv"
