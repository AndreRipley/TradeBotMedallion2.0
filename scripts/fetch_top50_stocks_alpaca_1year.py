"""
Fetch top 50 stocks by market cap at 30-minute intervals for the past 1 year using Alpaca API.
...
# Top 50 stocks by market cap (as of 2025)
TOP_50_STOCKS = [
...
def fetch_top50_alpaca_1year():
    """Fetch 1 year of 30-minute data for top 50 stocks."""
    ...
    try:
        ...
        for i, symbol in enumerate(TOP_50_STOCKS, 1):
            print(f"[{i}/{len(TOP_50_STOCKS)}] {symbol}:", end=' ')
        ...
        # Save combined data
        output_file = f"top50_stocks_30min_2years_alpaca_{end_date.strftime('%Y%m%d')}.csv"
        ...
        output_dir = f"top50_stocks_30min_2years_alpaca_{end_date.strftime('%Y%m%d')}"
