#!/bin/bash
# Deploy Trading Alert System to REPLACE existing trading-bot service

set -e  # Exit on error

echo "=========================================="
echo "REPLACING TRADING BOT WITH ALERT SYSTEM"
echo "=========================================="
echo ""
echo "⚠️  WARNING: This will replace the existing 'trading-bot' service"
echo ""
...
SERVICE_NAME="trading-bot"  # Same name to replace existing service
IMAGE_NAME="gcr.io/${PROJECT_ID}/${SERVICE_NAME}"

echo "Project ID: $PROJECT_ID"
echo "Region: $REGION"
echo "Service Name: $SERVICE_NAME (will replace existing)"
echo ""
...
# Confirm replacement
echo ""
read -p "⚠️  This will REPLACE the existing trading-bot service. Continue? (yes/no): " CONFIRM
if [ "$CONFIRM" != "yes" ]; then
...
echo "Using Dockerfile.alert for alert system..."
...
echo ""
echo "Step 4: Deploying to Cloud Run (REPLACING existing service)..."
...
echo "Region: $REGION"
echo ""
echo "⚠️  IMPORTANT: Old trading bot has been replaced!"
echo ""
echo "Service URL:"
...
echo ""
echo "   Required:"
echo "   - ALPHA_VANTAGE_API_KEY=your_api_key (optional, for real data)"
echo ""
...
echo "=========================================="
echo ""
echo "After deployment, initialize the system:"
echo ""
echo "Option 1: Use Cloud Shell"
echo "  gcloud run services exec $SERVICE_NAME --region=$REGION --command='python3 -m app.universe.build'"
echo ""
echo "Option 2: Run locally first, then deploy database"
echo "  python3 -m app.universe.build"
echo "  python3 -m app.indicators.compute_rsi --all"
echo ""
echo "View logs:"
